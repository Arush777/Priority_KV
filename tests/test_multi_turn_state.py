"""Tests for multi_turn_state templates."""

from __future__ import annotations

from prioritybench.generate import generate_one
from prioritybench.scoring import score_example
from prioritybench.templates import (
    MULTI_TURN_STATE_TEMPLATES_V1,
    MULTI_TURN_STATE_TEMPLATES_V2,
)


def test_multi_turn_gold_scores_one():
    for tmpl in MULTI_TURN_STATE_TEMPLATES_V1 + MULTI_TURN_STATE_TEMPLATES_V2:
        ex = generate_one(tmpl, seed=3, context_length=8000)
        line = ex.scoring["required_slots"]["line"]
        assert score_example(ex, line) == 1.0
        assert score_example(ex, "wrong") == 0.0


def test_v2_final_does_not_leak_gold():
    for tmpl in MULTI_TURN_STATE_TEMPLATES_V2:
        ex = generate_one(tmpl, seed=11, context_length=8000)
        gold = ex.scoring["required_slots"]["line"]
        final = ex.messages[-1]["content"]
        # Gold value must not appear verbatim in the FINAL ask.
        payload = gold.split("=", 1)[-1]
        assert payload not in final, (tmpl.template_id, final, gold)
